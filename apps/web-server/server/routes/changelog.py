...
def _require_safe_git_date(value: str, label: str) -> str:
    ...
    if (
        not isinstance(value, str)
        or value.startswith("-")
        or not _GIT_DATE_RE.fullmatch(value)
    ):
        raise HTTPException(status_code=400, detail=f"Invalid {label}")
    return value
    ...
    for task_id in request.taskIds:
        ...
        if spec_path.exists():
            try:
                ...
                logger.exception(
                    "Failed to read spec for task %s", sanitize_log(task_id)
                )
                specs.append(
...
async def read_existing_changelog(projectId: str = Path(...)):
    ...
    try:
        ...
        return {
            "success": True,
            "data": {"exists": True, "error": "Failed to read changelog"},
        }
        ...
        if mode == "branch-diff":
            ...
            if history_type == "last-n":
                try:
                    ...
                    raise HTTPException(
                        status_code=400, detail="Invalid commit count"
                    ) from None
