...
export interface ProjectSettings {
   ...
   * ``claude auth login`` on the TFactory host.
   */
...
export interface GraphitiProviderConfig {
  ...
  database?: string;  // Database name (default: tfactory_memory)
  dbPath?: string;    // Database storage path (default: ~/.tfactory/memories)
}
 ...
 * Stored in .tfactory/.env as AGENT_MCP_<agent>_ADD and AGENT_MCP_<agent>_REMOVE
 */
...
export interface AutoBuildVersionInfo {
  ...
  updateAvailable: boolean; // Always false - .tfactory only contains data, no code to update
}
