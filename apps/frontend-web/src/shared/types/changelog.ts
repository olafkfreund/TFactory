...
export interface TaskSpecContent {
  ...
  implementationPlan?: ImplementationPlan; // Parsed test_plan.json
...
export interface ChangelogSaveRequest {
  projectId: string;
  content: string;
  version: string; // Version to update in project files (package.json, pyproject.toml, etc.)
...
export interface ChangelogSaveResult {
  path: string; // Path to CHANGELOG.md
  version: string; // Version that was saved
  updatedFiles: string[]; // Files that were updated with the new version (e.g., ["package.json"])
}
