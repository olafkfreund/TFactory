...
export const useTerminalStore = create<TerminalState>((set, get) => ({
  ...
  reorderTerminals: (fromId: string, toId: string) => {
    set((state) => {
      ...
      const fromIndex = newTerminals.findIndex(t => t.id === fromId);
      const toIndex = newTerminals.findIndex(t => t.id === toId);
...
export async function restoreTerminalSessions(projectPath: string): Promise<void> {
  ...
  try {
    ...
    if (projectTerminals.length > 0) {
      ...
      const aliveChecks = await Promise.all(
        projectTerminals.map(async (terminal) => {
          try {
            const result = await window.API.checkTerminalPtyAlive(terminal.id);
    ...
    // Restore from disk
    const result = await window.API.getTerminalSessions(projectPath);
