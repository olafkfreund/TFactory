...
export const useFileExplorerStore = create<FileExplorerState>((set, get) => ({
  ...
  loadDirectory: async (dirPath: string): Promise<FileNode[]> => {
    ...
    try {
      ...
      // Transform backend response to FileNode[]
      // Backend returns: { path, entries: [{name, path, type, ...}], parent }
      // We need: FileNode[] with { path, name, isDirectory }
      const rawData = result.data as unknown as { entries?: Array<{ name: string; path: string; type: string }> } | Array<{ name: string; path: string; type: string }>;

      // Handle both array format (Electron) and object format (web API)
      const entries = Array.isArray(rawData) ? rawData : (rawData.entries || []);

      const fileNodes: FileNode[] = entries.map((entry: { name: string; path: string; type: string }) => ({
        path: entry.path,
        name: entry.name,
      ...
      set((state) => {
        ...
        newFiles.set(dirPath, fileNodes);
        const newLoading = new Map(state.isLoading);
