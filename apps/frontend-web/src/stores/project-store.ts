 ...
 * ``~/.tfactory/workspaces/`` on laptops; a PVC on K8s) and registers
...
/**
 * Check tfactory version status for a project
 */
...
/**
 * Initialize tfactory in a project
 */
  ...
  try {
    ...
    if (result.success && result.data) {
      ...
      if (result.data.success) {
        console.log('[ProjectStore] Updating project autoBuildPath to .tfactory');
        store.updateProject(projectId, { autoBuildPath: '.tfactory' });
      } else {
