...
import { checkGitHubConnection as checkGitHubConnectionGlobal } from '../../../stores/github/index';
import type {
  Project,
...
export function useProjectSettings(
  ...
  useEffect(() => {
    const loadEnvConfig = async () => {
      if (open && project.autoBuildPath) {
        ...
        setEnvError(null);
        try {
          const result = await window.API.getProjectEnv(project.id);
  ...
  useEffect(() => {
    const checkAuth = async () => {
      if (open && project.autoBuildPath) {
        ...
        try {
          const result = await window.API.checkClaudeAuth(project.id);
          if (result.success && result.data) {
  ...
  const handleSaveEnv = async () => {
    ...
    setEnvError(null);
    try {
      const result = await window.API.updateProjectEnv(project.id, envConfig);
      if (!result.success) {
  ...
  const handleClaudeSetup = async () => {
    setIsCheckingClaudeAuth(true);
    try {
      const result = await window.API.invokeClaudeSetup(project.id);
  ...
  const handleSave = async (onClose: () => void) => {
    ...
    try {
      ...
      if (envConfig) {
        const envResult = await window.API.updateProjectEnv(project.id, envConfig);
  ...
  const updateEnvConfig = async (updates: Partial<ProjectEnvConfig>) => {
    if (envConfig) {
      ...
      try {
        const result = await window.API.updateProjectEnv(project.id, newConfig);
        if (!result.success) {
