...
export function useGitHubInvestigation(projectId: string | undefined) {
  ...
  useEffect(() => {
    ...
    const cleanupProgress = window.API.onGitHubInvestigationProgress(
    ...
    const cleanupComplete = window.API.onGitHubInvestigationComplete(
    ...
    const cleanupError = window.API.onGitHubInvestigationError(
