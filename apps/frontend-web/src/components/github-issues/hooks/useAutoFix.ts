...
export function useAutoFix(projectId: string | undefined) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [configResult, queueResult, batchesResult] = await Promise.all([
        window.API.github.getAutoFixConfig(projectId),
        window.API.github.getAutoFixQueue(projectId),
        window.API.github.getBatches(projectId),
      ]);
  ...
  useEffect(() => {
    ...
    const cleanupComplete = window.API.github.onAutoFixComplete(
      (eventProjectId: string) => {
        if (eventProjectId === projectId) {
          window.API.github.getAutoFixQueue(projectId).then(setQueue);
        }
      }
    );
  ...
  useEffect(() => {
    ...
    const cleanupProgress = window.API.github.onBatchProgress(
    ...
    const cleanupComplete = window.API.github.onBatchComplete(
    ...
    const cleanupError = window.API.github.onBatchError(
  ...
  const saveConfig = useCallback(
    async (newConfig: AutoFixConfig): Promise<boolean> => {
      ...
      try {
        const success = await window.API.github.saveAutoFixConfig(projectId, newConfig);
        if (success) {
          setConfig(newConfig);
        }
        return success;
  ...
  const startBatchAutoFix = useCallback(
    (issueNumbers?: number[]) => {
      ...
      window.API.github.batchAutoFix(projectId, issueNumbers);
    },
    [projectId]
  );
  ...
  useEffect(() => {
    ...
    autoFixIntervalRef.current = setInterval(async () => {
      try {
        ...
        const newIssues = await window.API.github.checkNewIssues(projectId);
        if (newIssues.length > 0) {
          ...
          for (const issue of newIssues) {
            ...
            window.API.github.startAutoFix(projectId, issue.number);
          }
        }
      } catch (error) {
