...
export function useTerminalEvents({
  ...
  useEffect(() => {
    const cleanup = window.API.onTerminalOutput((id, data) => {
      if (id === terminalId) {
  ...
  useEffect(() => {
    const cleanup = window.API.onTerminalExit((id, exitCode) => {
      if (id === terminalId) {
  ...
  useEffect(() => {
    const cleanup = window.API.onTerminalTitleChange((id, title) => {
      if (id === terminalId) {
  ...
  useEffect(() => {
    const cleanup = window.API.onTerminalClaudeSession((id, sessionId) => {
      if (id === terminalId) {
