...
def setup_environment() -> Path:
    ...
    """
    # Add tfactory directory to path for imports
    script_dir = Path(__file__).parent.parent.resolve()
    sys.path.insert(0, str(script_dir))

    # Load .env file - check both tfactory/ and dev/tfactory/ locations
    env_file = script_dir / ".env"
    dev_env_file = script_dir.parent / "dev" / "tfactory" / ".env"
    if env_file.exists():
...
def find_spec(project_dir: Path, spec_identifier: str) -> Path | None:
    ...
    worktree_base = project_dir / ".tfactory" / "worktrees" / "tasks"
    if worktree_base.exists():
        # Try exact match in worktree
        worktree_spec = (
            worktree_base / spec_identifier / ".tfactory" / "specs" / spec_identifier
        )
        ...
        for worktree_dir in worktree_base.iterdir():
            if worktree_dir.is_dir() and worktree_dir.name.startswith(
                ...
                spec_in_worktree = (
                    worktree_dir / ".tfactory" / "specs" / worktree_dir.name
                )
                if (
