...
import { __tfTotp } from "./tftotp.helper";

const STORAGE_STATE = "@@STORAGE_STATE_PATH@@";

// RFC-6238 TOTP (SHA-1, 30s, 6 digits) for Class B MFA. __tfTotp lives in
// ./tftotp.helper.ts (#1063) — a plain module with no template placeholders,
// so it stays inside CodeQL's JS analysis (this template file is excluded, see
// .github/codeql/codeql-config.yml). render_auth_setup_steps() copies that
// helper alongside this rendered file into the generated test workspace.
