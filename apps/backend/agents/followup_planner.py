...
Runs a follow-up planning session that adds new subtasks to an *already
completed* spec — the AIFactory-style "add more work to this spec" flow.

This is distinct from the TFactory pipeline Planner (``planner.py``), which
turns an acceptance-criteria spec + diff into a fresh lane-tagged test plan.
The two share no logic; they were split out of a single module so each has
