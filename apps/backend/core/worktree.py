...
- Worktree path: .tfactory/worktrees/tasks/{spec-name}/
- Branch name: tfactory/{spec-name}
...
class WorktreeManager:
    ...
    Each spec gets its own worktree in .tfactory/worktrees/tasks/{spec-name}/ with
    a corresponding branch tfactory/{spec-name}.
    """

    def __init__(self, project_dir: Path, base_branch: str | None = None):
        self.project_dir = project_dir
        self.base_branch = base_branch or self._detect_base_branch()
        self.worktrees_dir = project_dir / ".tfactory" / "worktrees" / "tasks"
    ...
    def _unstage_gitignored_files(self) -> None:
        ...
        plus any files in the .tfactory directory which should never be merged.

        This is needed after a --no-commit merge because files that exist in the
        source branch (like spec files in .tfactory/specs/) get staged even if
        ...
        # Files to unstage: gitignored files + .tfactory directory files
        ...
        # 2. Always unstage .tfactory directory files - these are project-specific
        # and should never be merged from the worktree branch
        magestic_ai_patterns = [".tfactory/", "tfactory/specs/"]
        # Root-level runtime files that agents may commit despite .gitignore
        magestic_ai_root_files = {".tfactory-security.json", ".tfactory-status"}
    ...
    def _check_branch_namespace_conflict(self) -> str | None:
        """
        Check if a branch named 'tfactory' exists, which would block creating
        branches in the 'tfactory/*' namespace.

        Git stores branch refs as files under .git/refs/heads/, so a branch named
        'tfactory' creates a file that prevents creating the 'tfactory/'
        directory needed for 'tfactory/{spec-name}' branches.

        Returns:
            The conflicting branch name if found, None otherwise.
        """
        result = self._run_git(["rev-parse", "--verify", "tfactory"])
        if result.returncode == 0:
            return "tfactory"
        return None
    ...
    def create_worktree(self, spec_name: str) -> WorktreeInfo:
        ...
        # Check for branch namespace conflict (e.g., 'tfactory' blocking 'tfactory/*')
        ...
        if conflicting_branch:
            raise WorktreeError(
                ...
                f"\n"
                f"Git branch names work like file paths - a branch named 'tfactory' prevents\n"
                f"creating branches under 'tfactory/' (like 'tfactory/{spec_name}').\n"
                f"\n"
    ...
    def merge_worktree(
        ...
        if no_commit:
            ...
            merge_args.extend(["-m", f"tfactory: Merge {info.branch}"])
    ...
    def commit_in_worktree(self, spec_name: str, message: str) -> bool:
        ...
        self._run_git(["add", ".", ":!.tfactory"], cwd=worktree_path)
    ...
    def list_all_spec_branches(self) -> list[str]:
        """List all tfactory branches (even if worktree removed)."""
        result = self._run_git(["branch", "--list", "tfactory/*"])
        if result.returncode != 0:
            return []
