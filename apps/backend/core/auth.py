"""
Authentication helpers for TFactory.

Provides centralized authentication token resolution with fallback support
for multiple environment variables, TFactory profiles, Claude Code CLI
...
def _get_token_from_tfactory_profiles() -> str | None:
    """Read token from TFactory profiles storage (~/.tfactory/claude-profiles.json)."""
    path = Path.home() / ".tfactory" / "claude-profiles.json"
    if not path.exists():
        return None
    try:
...
def get_auth_token() -> str | None:
    ...
    3. TFactory profiles (~/.tfactory/claude-profiles.json)
    ...
    # Check TFactory profiles
    token = _get_token_from_tfactory_profiles()
    if token:
        return token
...
def get_auth_token_source() -> str | None:
    ...
    # Check TFactory profiles
    if _get_token_from_tfactory_profiles():
        return "TFactory Profiles"
...
def require_auth_token() -> str:
    ...
    if not token:
        error_msg = (
            ...
            "TFactory requires Claude Code OAuth authentication.\n"
