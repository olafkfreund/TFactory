...
ccstatusline Configuration:
    Add to ~/.config/ccstatusline/settings.json:
    {
        "widgets": [
            {
                "type": "custom_command",
                "command": "python /path/to/tfactory/statusline.py",
                "refresh": 5000
            }
        ]
    }
...
def find_project_root() -> Path:
    """Find the project root by looking for .tfactory or .tfactory-status."""
    cwd = Path.cwd()

    # Check current directory - prioritize .tfactory (installed instance)
    if (cwd / ".tfactory").exists():
        return cwd
    if (cwd / ".tfactory-status").exists():
        return cwd

    # Walk up to find project root
    for parent in cwd.parents:
        if (parent / ".tfactory").exists():
            return parent
        if (parent / ".tfactory-status").exists():
            return parent
