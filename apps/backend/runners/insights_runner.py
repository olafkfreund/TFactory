...
def load_project_context(project_dir: str) -> str:
    ...
    # Load project index if available (from .tfactory - the installed instance)
    index_path = Path(project_dir) / ".tfactory" / "project_index.json"
    ...
    roadmap_path = Path(project_dir) / ".tfactory" / "roadmap" / "roadmap.json"
    ...
    # Load existing tasks
    tasks_path = Path(project_dir) / ".tfactory" / "specs"
