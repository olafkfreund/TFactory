...
def test_known_language_unchanged_by_manifest_param():
    assert get_tool_for_lane("python", "unit").binary == "pytest"
    # manifest is ignored for a registry language
    assert (
        get_tool_for_lane(
            "python", "unit", manifest={"verify_commands": ["nope"]}
        ).binary
        == "pytest"
    )
...
def test_unknown_language_with_manifest_synthesizes_ondemand_spec():
    spec = get_tool_for_lane(
        "elixir",
        "unit",
        manifest={"language": "elixir", "verify_commands": ["mix test", "mix credo"]},
    )
...
def test_unknown_language_without_manifest_still_raises_honestly():
    with pytest.raises(UnsupportedLanguageError):
        get_tool_for_lane("cpp", "unit")
    with pytest.raises(UnsupportedLanguageError):
        get_tool_for_lane("cpp", "unit", manifest={"verify_commands": []})  # no path
