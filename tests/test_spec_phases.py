...
Tests the PhaseExecutor class in tfactory/spec/phases.py covering:
...
class TestPhaseResult:
    ...
    def test_phase_result_multiple_output_files(self):
        ...
        result = PhaseResult(
            phase="spec_writing",
            success=True,
            output_files=["spec.md", "test_plan.json"],
            errors=[],
            retries=0,
        )
...
class TestPhaseQuickSpec:
    ...
    async def test_quick_spec_files_exist(
        ...
        (spec_dir / "test_plan.json").write_text(json.dumps({"phases": []}))
...
class TestPhasePlanning:
    ...
    async def test_planning_file_exists_valid(
        ...
        (spec_dir / "test_plan.json").write_text(json.dumps({
