...
class TestFileBasedEndpoints:
    ...
    def test_update_api_key_success(
        self, client, mock_file_system, test_data_factory, assert_endpoint
    ):
        """
    ...
    def test_update_api_key_invalid(self, client, test_data_factory, assert_endpoint):
        """Test API key validation rejects invalid keys."""
        # Arrange
        request_data = test_data_factory.settings_api_key_request(api_key="invalid-key")
        ...
        assert_endpoint.assert_error_response(
            response, expected_status=400, expected_message="Invalid API key format"
        )

    def test_set_active_profile(
        self, client, mock_claude_profiles, test_data_factory, assert_endpoint
    ):
        """
    ...
    def test_update_feature_status(
        self, client, mock_roadmap_json, test_data_factory, assert_endpoint
    ):
        """
        ...
        request_data = test_data_factory.roadmap_feature_status_request(
            feature_id="feature-1", status="completed"
        )

        # Act
        response = client.patch(
            "/api/roadmap/features/feature-1/status", json=request_data
        )
...
class TestCLIIntegrationEndpoints:
    ...
    def test_update_merge_request(
        self, client, mock_glab_cli, test_data_factory, assert_endpoint
    ):
        """
        ...
        request_data = test_data_factory.gitlab_mr_update_request(
            mr_id=123, title="Updated MR Title", description="Updated description"
        )
    ...
    def test_approve_merge_request(self, client, mock_glab_cli, assert_endpoint):
        """
    ...
    def test_squash_commits(self, client, mock_subprocess, assert_endpoint):
        """
...
class TestAIServiceEndpoints:
    ...
    async def test_generate_ideation(
        self, client, mock_ai_service, mock_background_task, assert_endpoint
    ):
        """
    ...
    async def test_investigate_gitlab_issue(
        self, client, mock_glab_cli, mock_ai_service, assert_endpoint
    ):
        """
        ...
        mock_glab_cli.mock_command(
            "issue view 456 --json", output=json.dumps(issue_data)
        )
        mock_ai_service.configure_response(
...
class TestEndpointIntegrationWorkflows:
    ...
    def test_profile_management_workflow(self, endpoint_integration_helper):
        """
        ...
        response = helper.client.post(
            "/api/settings/profiles", json={"name": "New Profile"}
        )
        ...
        response = helper.client.post(
            "/api/settings/profiles/active", json={"profile_id": profile_id}
        )
        helper.assert_endpoint.assert_success_response(response)

        # Step 4: Rename profile
        response = helper.client.patch(
            f"/api/settings/profiles/{profile_id}", json={"name": "Production Profile"}
        )
        helper.assert_endpoint.assert_success_response(response)

    def test_roadmap_workflow(
        self, endpoint_integration_helper, mock_roadmap_json, mock_ideation_json
    ):
        """
        ...
        response = helper.client.patch(
            "/api/roadmap/ideas/idea-1/status", json={"status": "accepted"}
        )
        helper.assert_endpoint.assert_success_response(response)

        # Step 3: Update feature status
        response = helper.client.patch(
            "/api/roadmap/features/feature-1/status", json={"status": "completed"}
        )
        helper.assert_endpoint.assert_success_response(response)
...
class TestEndpointErrorHandling:
    ...
    def test_file_not_found_handling(self, client, assert_endpoint):
        """Test graceful handling when config files don't exist."""
        # Act - try to update non-existent profile
        response = client.patch(
            "/api/settings/profiles/nonexistent/token", json={"token": "test"}
        )

        # Assert - should return appropriate error
        assert_endpoint.assert_error_response(
            response, expected_status=404, expected_message="Profile not found"
        )

    def test_cli_tool_not_available(self, client, mock_subprocess, assert_endpoint):
        ...
        # Assert
        assert_endpoint.assert_error_response(
            response, expected_status=500, expected_message="glab CLI not available"
        )

    def test_invalid_json_file_handling(
        self, client, mock_file_system, assert_endpoint
    ):
        ...
        # Assert
        assert_endpoint.assert_error_response(
            response, expected_status=500, expected_message="Failed to read profiles"
        )
...
class TestEndpointSecurity:
    ...
    def test_api_key_not_logged(self, client, caplog, test_data_factory):
    ...
    def test_token_secure_storage(self, client, mock_file_system, test_data_factory):
        """Ensure tokens are stored with appropriate permissions."""
        # Arrange
        request_data = test_data_factory.settings_profile_token_request(
            profile_id="profile-1", token="sk-ant-secret-token"
        )

        # Act
        response = client.patch(
            "/api/settings/profiles/profile-1/token", json=request_data
        )
