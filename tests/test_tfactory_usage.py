...
def test_add_and_event_block():
    u = RunUsage(input_tokens=100, output_tokens=20, cost_usd=0.01, model="claude-sonnet-4-6")
...
def test_estimate_cost_known_and_unknown_model():
    assert estimate_cost_usd(1_000_000, 1_000_000, "claude-sonnet-4-6") == 18.0  # 3 + 15
...
def test_usage_from_sdk_result_message_uses_real_cost():
    ...
    assert (u.input_tokens, u.output_tokens, u.cost_usd, u.model) == (321, 88, 0.5, "claude-sonnet-4-6")


def test_usage_from_result_message_estimates_cost_when_absent():
    msg = _FakeResultMessage(1_000_000, 0, model="claude-sonnet-4-6")  # no total_cost_usd
    u = usage_from_obj(msg)
    assert u.cost_usd == 3.0  # 1M input @ $3/Mtok


def test_usage_from_dict_and_none_and_zero():
    assert usage_from_obj({"input_tokens": 10, "output_tokens": 2, "model": "x"}).input_tokens == 10
...
def test_record_in_status_accumulates_across_sessions(tmp_path: Path):
    ...
    record_in_status(tmp_path, RunUsage(input_tokens=100, output_tokens=10, cost_usd=0.1, model="m"))
    record_in_status(tmp_path, RunUsage(input_tokens=50, output_tokens=5, cost_usd=0.2))  # a retry
...
def test_usage_block_zeros_when_nothing_recorded(tmp_path: Path):
    assert usage_block_from_status(tmp_path) == {
        "input_tokens": 0, "output_tokens": 0, "total_tokens": 0, "cost_usd": 0.0, "model": "",
    }
...
def test_envelope_carries_zero_usage_by_default(tmp_path: Path):
    (tmp_path / "status.json").write_text(json.dumps({"task_id": "042", "status": "triaged"}))
...
def test_envelope_reflects_recorded_usage(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("TFACTORY_COMPLETION_SENTINEL", "1")
    (tmp_path / "findings").mkdir(parents=True, exist_ok=True)
    (tmp_path / "status.json").write_text(json.dumps({"task_id": "042", "status": "triaging"}))
    record_in_status(tmp_path, RunUsage(input_tokens=2400, output_tokens=100, cost_usd=1.25, model="claude-sonnet-4-6"))
