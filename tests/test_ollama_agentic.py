...
# The SDK stand-in belongs to tests/conftest.py, which installs one only when
# claude_agent_sdk is genuinely absent (#882). A module-local MagicMock here
# won that race on an INSTALLED SDK and leaked a permissive fake into
...
# different model can actually run these instead of always skipping (PFactory#418).
_LIVE_MODEL = os.environ.get("TFACTORY_OLLAMA_TEST_MODEL", _LIVE_MODEL)
...
_skip_no_ollama = pytest.mark.skipif(
    ...
    reason=(
        ...
        f"Override with TFACTORY_OLLAMA_TEST_MODEL."
    ),
)
...
class TestOllamaAgenticLive:
    ...
    def test_live_read_file(self, provider, tmp_path):
        ...
        assert (
            ...
            or "tfactory" in final_text.lower()
