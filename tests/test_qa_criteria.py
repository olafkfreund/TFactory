...
# =============================================================================
# MOCK SETUP - Must happen before ANY imports from tfactory
# =============================================================================
...
# Now we can safely add the tfactory path and import.
...
class TestImplementationPlanIO:
    ...
    def test_load_test_plan(self, spec_dir: Path, sample_test_plan: dict):
        """Loads implementation plan from JSON."""
        plan_file = spec_dir / "test_plan.json"
        plan_file.write_text(json.dumps(sample_test_plan))
    ...
    def test_save_test_plan(self, spec_dir: Path):
        """Saves implementation plan to JSON."""
        plan = {"feature": "Test", "phases": []}

        result = save_test_plan(spec_dir, plan)

        assert result is True
        assert (spec_dir / "test_plan.json").exists()

        loaded = json.loads((spec_dir / "test_plan.json").read_text())
        assert loaded["feature"] == "Test"

    def test_save_test_plan_creates_file(self, spec_dir: Path):
