...
> Design document for converting TFactory from Electron to a containerized web application.

## Executive Summary

This document outlines the architecture for a Docker-native version of TFactory that:
...
| POST | `/api/projects/{id}/initialize` | Initialize tfactory in project |
| GET | `/api/projects/{id}/version` | Check tfactory version |
...
WORKDIR /app
COPY tfactory/requirements.txt .
RUN pip install --no-cache-dir -r requirements.txt

# Install API dependencies
COPY tfactory/api/requirements.txt ./api-requirements.txt
RUN pip install --no-cache-dir -r api-requirements.txt

# Copy application code
COPY tfactory/ ./tfactory/

# --- Frontend Build Stage ---
FROM node:22-alpine AS frontend-build

WORKDIR /app
COPY tfactory-ui/package*.json ./
RUN npm ci
...
services:
  app:
    ...
    volumes:
      ...
      - tfactory-claude:/home/claude/.claude
    environment:
  ...
  falkordb:
    ...
    container_name: tfactory-falkordb
    volumes:
      - falkordb-data:/data
    networks:
      - tfactory-net
  ...
  graphiti-mcp:
    ...
    container_name: tfactory-graphiti
...
1. OAuth tokens stored in named volume (`tfactory-claude`)
